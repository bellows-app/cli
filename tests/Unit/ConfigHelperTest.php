...
it('will create a new file if the file is not found and add the config', function () {
    ...
    expect(getConfigContents('nada'))->toBe(
        <<<'CONFIG'
        <?php
...
it('can write a value to a config in a subdirectory', function () {
    writeToConfig(
        <<<'CONFIG'
        <?php
    ...
    expect(getConfigContents('statamic/users'))->toBe(
        <<<'CONFIG'
        <?php
