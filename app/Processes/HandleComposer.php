...
class HandleComposer
{
    ...
    {
        Console::step('Composer Packages');
        ...
        collect($installation->config->get('composer-scripts', []))->whenNotEmpty(
            fn ($scripts) => $scripts->each(fn ($commands, $event) => Composer::addScript($event, $commands)),
        );

        $installation->manager->allowedComposerPlugins($installation->config->get('composer-allow-plugins', []))->whenNotEmpty(
            fn ($packages) => Composer::allowPlugin($packages->toArray()),
        );

        $installation->manager->composerPackages($installation->config->get('composer', []))->whenNotEmpty(
            function ($packages) {
        ...
        $installation->manager->composerDevPackages($installation->config->get('composer-dev', []))->whenNotEmpty(
            function ($packages) {
                [$withFlags, $noFlags] = $packages->partition(fn ($package) => Str::contains($package, ' --'));
